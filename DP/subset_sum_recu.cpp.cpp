...
bool subset_sum_rec(int wt[],int W,int n)
{
    if(W==0)
        return true;
    else if(n==0 && W!=0)
    ...
    if(wt[n-1]>W)
        return subset_sum_rec(wt,W,n-1);

    else
        return subset_sum_rec(wt,W-wt[n-1],n-1) || subset_sum_rec(wt,W,n-1);
}

signed main()
{
    int n,W;
    cin>>n>>W;///here W is the capacity of the bag
